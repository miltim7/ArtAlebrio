      ...
      <div class="video-content">
        ...
        <div class="video-url-input-container">
          <input 
            ...
            placeholder="Вставьте ссылку на видео (YouTube, ВКонтакте, Vimeo)"
        ...
        <div class="upload-description">
          <p>Допустимые форматы: mp4, ogy, webm; размер до 20Mb</p>
          
          <p>Оживите свой продукт с помощью видеоролика продолжительностью от 5 до 15 секунд. Это может помочь вам увеличить ваши продажи. В видео не будет звука, поэтому пусть говорит ваш продукт</p>
          
          <p>Настройте параметры для записи с высоким разрешением - стремитесь к 1080p</p>
          
          <p>Обережите ваше видео по завершении загрузки</p>
...
export default {
  ...
  methods: {
    ...
    async getYouTubeThumbnail(videoId) {
      // Пробуем разные качества превью
    ...
    async getVKThumbnail(videoId) {
      // Для VK используем стандартную заглушку или попробуем получить через API
      // В реальном проекте лучше использовать VK API
      const placeholderUrl = `https://vk.com/images/video_placeholder.png`;
      
      // Временное решение - возвращаем заглушку VK
    ...
    async getVimeoThumbnail(videoId) {
      try {
        // Используем Vimeo API для получения thumbnail
